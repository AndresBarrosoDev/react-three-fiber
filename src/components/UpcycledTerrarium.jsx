...
Command: npx gltfjsx@6.2.18 public/models/upcycled_terrarium.glb 
...
export function UpcycledTerrarium(props) {
  const group = useRef()
  const { nodes, materials, animations } = useGLTF('./models/upcycled_terrarium.glb')
  const { actions } = useAnimations(animations, group)

  
  useEffect(() => {
    // Suponiendo que hay una animación principal que queremos reproducir
    if (actions && actions['Animation']) {
      actions['Animation'].play();
    }
    // Puedes usar `Object.keys(actions)` para ver los nombres de las animaciones disponibles
    // y sustituir 'AnimationName' con el nombre correcto de la animación que quieres reproducir.
  }, [actions]);
  
  return (
    <group ref={group} {...props} dispose={null} position={[-0.14, -0.8, 0]} rotation={[0, 0, 0]}>
      <group name="Sketchfab_aScene" position={[0, 0, 0]} rotation={[0, 0, 0]}>
        <group name="Sketchfab_model" rotation={[-Math.PI / 2, 0, 0]}>
          <group name="root" position={[0, 0, 0]} rotation={[0, 0, 0]}>
...
useGLTF.preload('./models/upcycled_terrarium.glb')
